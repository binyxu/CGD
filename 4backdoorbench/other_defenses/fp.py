...
class FinePrune(defense):
    ...
    def set_args(self, parser):
        ...
        parser.add_argument('--enable_clip', type=int, default=0, help='0 disable, 1 enable')
        return parser
    ...
    def process_args(self, args):
        ...
        # HERE 3. modify output
        if args.enable_clip == 1:
            defense_save_path = "record" + os.path.sep + args.result_file + os.path.sep + "defense" + os.path.sep + "clip_fp"
        else:
    ...
    def defense(self):
        ...
        # HERE 1. add new choose_index from bd dataset
        if args.enable_clip == 0:
            ...
            import copy
            bd_dataset = copy.deepcopy(attack_result['bd_train'].wrapped_dataset)
            with open(f"./record/{args.result_file}/AWC/clean_indices.json", 'r') as json_file:
                import json
                loaded_list = json.load(json_file)
            bd_dataset.subset(loaded_list)
            clean_train_dataset_without_transform = bd_dataset
            clean_train_dataset_with_transform.wrapped_dataset = clean_train_dataset_without_transform
